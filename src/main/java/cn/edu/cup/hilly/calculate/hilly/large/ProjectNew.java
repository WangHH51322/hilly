...
//    public double[][] h2_his = new double[varPara.i][varPara.k];     //上坡液位  历史数据
...
//        for (int s = 0; s < varPara.staV.size(); s++) {
//            System.out.println("s = " + s);
//            System.out.println("varPara.staV = " + varPara.staV.get(s));
//        }
...
//        }
        ...
        for (int x = 1; x <=(varPara.line_l[varPara.i-1][3]-varPara.line_l[1][1])/500; x++) {
            ...
            varPara.dHL_new[0][x]=0.5+varPara.dHL_new[0][x-1];//地形的里程
            lz_new[x][0]= varPara.dHL[0][x];
        }
        for (int x = 1; x <=(varPara.line_l[varPara.i-1][3]-varPara.line_l[1][1])/500; x++) {
            varPara.dHL[varPara.dHL.length-2][x]=getZ(varPara.line_l, varPara.line_d,varPara.dHL_new[0][x]*1000);
            varPara.dHL_new[varPara.dHL.length-2][x]=getZ(varPara.line_l, varPara.line_d,varPara.dHL_new[0][x]*1000);
        ...
        conPara.Ql = varPara.Qh/3600.0;
        for(int kk=1; kk<varPara.kt+1; kk++){       //时间循环，从第一个u型管低点开始，到水头遇终点结束。
...
//                varPara.Qh2k[i]=varPara.h2k[i];
//                varPara.Qh1k[i]=varPara.h1k[i];
//                varPara.Quk[i]=varPara.uk[i];
//                varPara.Qlgk[i]=varPara.lgk[i];
//                varPara.QPgk[i]=varPara.Pgk[i];
//                varPara.QMGk[i]=varPara.MGk[i];
//                varPara.QDengk[i]=varPara.Dengk[i];
//                varPara.QdMg_in[i]=varPara.dMg_in[i];
//                varPara.QdMg_out[i]=varPara.dMg_out[i];

                //计算本时步的所有管段的初次压缩

//                if (varPara.h2k[i] < 300 && varPara.ssr[i][0]!=1)            //20201105加入后半个判定条件，让破碎结束后不再运行，前半个条件是预设的破碎开始情况
//                {
...
//                            varPara.dMg_in[i]-varPara.dMg_out[i]);
//                    varPara.MGk[i]=varPara.MGk[i]+varPara.dMg_in[i];//在破碎未开始的时间，更新气段质量,主要是上游流下的气体
//                    //计算当前压力下的气体密度
...
//                    if (varPara.Pgk[i] < 0.1*oil.getDensity()*conPara.g*(varPara.line_l[i][3]-varPara.line_l[i][2])&&varPara.Pgk[i] <200000 ){
                    if (varPara.Pgk[i] < 0.1*oil.getDensity()*conPara.g*(varPara.line_l[i][3]-varPara.line_l[i][2])&&varPara.Pgk[i] <(conPara.Ql*3600/869.0)*200000 ){
//                        YaSuo(i, varPara.lgk[i], varPara.Pgk[i], varPara.Hgk[i], varPara.slopeU[i][0],
//                                varPara.slopeD[i][0], varPara.uk[i], varPara.h1k[i], varPara.h2k[i], varPara.backPressure[i],
...
//                    else if (varPara.Pgk[i] < 5e5+0.1*oil.getDensity()*conPara.g*(varPara.line_l[i][3]-varPara.line_l[i][2])) {
//                        varPara.Dengk[i] = 1.205 / 1.01e5 * varPara.Pgk[i];
//                        try {
//                            dtPosui(kk, i, varPara.l0[i], varPara.line_l[i][1], varPara.line_d[i][1], varPara.line_l[i][2], varPara.line_d[i][2],
//                                    conPara.Ql, D, 1.205,varPara.MGk[i]-AddMg[i] , varPara.Pgk[i], varPara.lgk[i],
//                                    varPara.Dengk[i], varPara.Hgk[i], varPara.MM, varPara.Pgk, varPara.lgk, varPara.Dengk, varPara.Hgk,
//                                    oil.density, 1e-6);//
//                            //if (i==4) System.out.println("varPara.Pgk 3 ="+varPara.Pgk[i]);
//                        } catch (IOException e) {
//                            e.printStackTrace();
//                        }
//                    }
                        ...
                        //varPara.h2k[i] = varPara.Qh2k[i] + varPara.slopeU[i][0]*conPara.Ql / A;
                        //varPara.h1k[i] = varPara.h1_his[i][(kk) / 300 - 1];
                        //System.out.println(varPara.Pg_his[i][(kk) / 300 - 1]);
                        //varPara.Pgk[i] = varPara.Pg_his[i][(kk) / 300 - 1];
                        //System.out.println(varPara.Pgk[i]);
//                        varPara.lgk[i] = 0.1;                //气体长度为零
                        //System.out.println(varPara.lgk[i]);
                        //varPara.lgk[i] = varPara.lg_his[i][(kk) / 300 - 1];
                        //System.out.println("lghis="+varPara.lg_his[i][(kk) / 300 - 1]);
                        //varPara.h1k[i] = varPara.line_d[i][1] - varPara.line_d[i][2];
                        //his_h2k=varPara.h2k[i];

//                        if (varPara.h2k[i] < varPara.line_d[i][3]-varPara.line_d[i][2]+0.1){
                        if ((kk) / 300 - 1<0){
...
//                        }
                        ...
                        //varPara.MGk[i] = varPara.M_his[i][(kk) / 300 - 1];
                    }
...
//                    }
                ...
                if (tt > 299) {
                    ...
                    if (varPara.allLine[3][rr]==0){
                        System.out.println("99999999999999999varPara.waterHeadLocation["+rr+"] = " + varPara.waterHeadLocation[rr]);
                    }
...
//                        if (num000>=1){
//                            System.out.println("(dpL[(int)((waterL-varPara.line_l[1][1])/500.0)-1]-getZ(varPara.line_l,varPara.line_d,waterL-500) = " + (varPara.dHL[varPara.num][(int)((varPara.waterL[varPara.num]-varPara.line_l[1][1])/500.0)-1]-getZ(varPara.line_l,varPara.line_d,varPara.waterL[varPara.num]-500)));
//                            System.out.println("(dpL[(int)((waterL-varPara.line_l[1][1])/500.0)]-getZ(varPara.line_l,varPara.line_d,waterL) = " + (varPara.dHL[varPara.num][(int)((varPara.waterL[varPara.num]-varPara.line_l[1][1])/500.0)]-getZ(varPara.line_l,varPara.line_d,varPara.waterL[varPara.num])));
//                            System.out.println("dpL_new[(int)((waterL-varPara.line_l[1][1])/500.0)-1] = " + varPara.dPL[varPara.num][(int)((varPara.waterL[varPara.num]-varPara.line_l[1][1])/500.0)-1]);
//                            System.out.println("dpL_new[(int)((waterL-varPara.line_l[1][1])/500.0)] = " + varPara.dPL[varPara.num][(int)((varPara.waterL[varPara.num]-varPara.line_l[1][1])/500.0)]);
//                            num000=0;
//                        }
                    //}

                    /**
                     * 20210723 新增运行中高点排气语句
                     */
...
//            varPara.dPL[varPara.num+3][x]=2499.385+varPara.dPL[varPara.num][x]/9.81/1000;
        ...
        try {
            ...
            Output.OutToTXT1(varPara.HFB,"HFB");
            Output.OutToTXT1(varPara.HFB1,"HFB1");
            Output.OutToTXT1(varPara.HFBU,"HFBU");
            Output.OutToTXT1(varPara.HFBU1,"HFBU1");
            Output.OutToTXT1(varPara.HFF,"HFF");
            Output.OutToTXT1(varPara.HFF1,"HFF1");
            Output.OutToTXTL(varPara.dPL,"dPL");
            Output.OutToTXTDHL(varPara.allLineLbz,"allLineLbz");
            Output.OutToTXTDHL(varPara.allLineDP,"allLineDP");
            //System.out.println(varPara.allLineLbzB[0].get(0));
            Output.OutToTXTLbz(varPara.allLineLbzBs,"allLineLbzBs");
...
//            Output.OutToTXTL(varPara.dPL,"dPL");
            ...
            Output.OutToTXT1(varPara.lg_f,"lg_f");
            Output.OutToTXT1(varPara.lp_b,"lp_b");
            Output.OutToTXT1(varPara.lp_bU,"lp_bU");
            Output.OutToTXT1(varPara.lg_his,"lg_his");
            Output.OutToTXT000(varPara.lg_his, varPara.line_l, "lg_new_his");
            Output.OutToTXTQ(varPara.allQ, "Q");
            Output.OutToTXT1(varPara.dMg_out_his,"dMg_out_his");
            Output.OutToTXT1(AddMg_his,"AddMg_his");
            Output.OutToTXT1(varPara.backPressure_his,"backPressure_his");
            Output.OutToTXT1(varPara.h2_his,"h2_his");

            //OutPut.OutToTXT1(varPara.hgs_his,"hgs_his");
            //OutPut.OutToTXT1(varPara.hgw_his,"hgw_his");
            //OutPut.OutToTXT1(varPara.hl_his,"hl_his");
            Output.OutToTXT1(varPara.M_his,"M_his");
            Output.OutToTXT1(varPara.allLine,"allLine");
            Output.OutToTXT1(varPara.Deng_his,"Deng_his");
            Output.OutToTXT(varPara.waterHeadLocation,"waterHeadLocation");
            Output.OutToTXT(varPara.oilHeadLocation,"oilLocation");
            Output.OutToTXTSimple(varPara.Hfk_lbz_D,"varPara.Hfk_lbz_D");
            Output.OutToTXTSimple(varPara.Hfk_lbz_U,"varPara.Hfk_lbz_U");
            Output.OutToTXTLLL(varPara.pigL[0],"pigL",flagpigT[0]);
            Output.OutToTXTLLL(varPara.pigL[1],"pigL1",flagpigT[1]);
            ...
            Output.OutToTXT123(varPara.dMg_p_stop,"dMg_p_stop");
            Output.OutToTXTLLL(varPara.pigV[0],"pigV",flagpigT[0]);
            Output.OutToTXTLLL(varPara.pigV[1],"pigV1",flagpigT[1]);
            Output.OutToTXTLLL(varPara.pigZ[1],"pigZ1",flagpigT[1]);
...
//            Output.OutToTXTZZZ(varPara.allLineStaticP[1],"allLineZ",varPara.line_l[1][1]);
            Output.OutToTXT(varPara.Hf,"Hf");
            Output.OutToTXT(varPara.Hf_b,"Hf_b");
            Output.OutToTXT(varPara.Hf_f,"Hf_f");
            Output.OutToTXT(varPara.Hf_j,"Hf_j");
            Output.OutToTXT(varPara.Time,"Time");

//            for (int iii=1;iii<varPara.i;iii++){
//                System.out.println("slopeD"+Math.toDegrees(varPara.slopeD[iii][0]));
//                System.out.println("slopeU"+Math.toDegrees(varPara.slopeU[iii][0]));
//            }
        } catch (IOException e) {
            e.printStackTrace();
        }
...
//            if (varPara.lg_his[i][k]!=0 && varPara.pigflag1==1 &&
//                    (varPara.lg_his[i][k]/varPara.lg_his[i][k-1]>5||varPara.lg_his[i][k]/varPara.lg_his[i][k-1]<-5))  {
//                varPara.lg_his[i][k]=varPara.lg_his[i][k-1];
//                //varPara.lgk[i]=1000*varPara.lg_his[i][k-1];
//            }
...
//            }
            ...
            varPara.HFB1[i][k] = varPara.Hfk_b1[i];
            varPara.HFF[i][k] = varPara.Hfk_f[i];
            varPara.HFF1[i][k] = varPara.Hfk_f1[i];
            varPara.HFBU[i][k] = varPara.Hfk_bU[i];
            varPara.HFBU1[i][k] = varPara.Hfk_bU1[i];
...
//    public void dHL_new(int n,double T,int num,double dx,double[] waterHeadLocation,double[] dpb,double[] dpbU,double[] dpf,double [][]lg_f,double []vll,double []stationL,double []stationP){
//        Pipeline pipeLine = pipeLines.get(0);
//        double D=pipeLine.getDiameter()-2*pipeLine.getThinkness();
//        double r=D/2;
//        double A=Math.PI*D*D/4.0;
...
//    }
    ...
    public double getPhi(double [][]arrL,double [][]arrZ,double l){
        ...
        if (Phi == 0) System.out.println("获取角度失败"+l);
        return Phi;
    }
    ...
    public int getI(double [][]arrL,double l){
        ...
        if (z == 0) System.out.println("获取段数失败"+l);
        return z;
    }
    ...
    public double getFlowPattern(int i,double L,double pigStart,double pigL,double flagpig){
        ...
        if (L<varPara.oilL[varPara.num] && varPara.oilL[varPara.num]!=0){
            p=4;
        }
    ...
    public void staticP(int n, double waterL,double dx) {
        ...
        if (waterL >= varPara.line_l[n][2] && waterL <= varPara.line_l[n][3]) {//水头在上坡段
            ...
            for (int x=(int)((waterL-varPara.line_l[1][1])/dx);x>=1;x--)//遍历水头前的各点,从后往前
            {
                ...
                if (i==0){
                    System.out.println("99999999999999999lxxxx["+Lx+"]");
                }
    ...
    public void staticP_new(int n, double waterL,double dx) {
        ...
        for (int s=0;s<varPara.pumpSta.size()-1;s++){        //改变站点压力值
            ...
            for (int x = 0;x< (int) ((waterL - varPara.line_l[1][1]) / dx +1 );  x++)//遍历水头前的各点,从后往前
            {
                ...
                for (int s=0;s<sta.size()-1;s++){        //改变站点压力值
                    if (Lx>=sta.get(s) && Lx <=sta.get(s+1)){
                        max0.set(s,varPara.dHL_new[varPara.dHL_new.length-2][x]+10);
                        if (max0.get(s)>maxZ.get(s)){
                            maxZ.set(s,max0.get(s));
                        }
                    }else if (Lx<sta.get(s)&&s==0){
...
//            varPara.dMg_p[1][iter+1]=varPara.dMg_p[1][iter]+zlll[iter]*30*varPara.deltaT;
//
